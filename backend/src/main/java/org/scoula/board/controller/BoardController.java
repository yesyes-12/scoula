...
//    }
    ...
    // ResponseEntity: 상태 코드/헤더/바디 설정 가능(Rest Controller 객체 리턴은 불가)
    @ApiOperation(value="게시글 목록", notes="게시글 목록 얻는 API")
    @ApiResponses(value= {
    ...
    public ResponseEntity<List<BoardDTO>> getList() {
        return ResponseEntity.ok(service.getList()); // 200 코드 응답
    }

    @ApiOperation(value="상세정보 얻기", notes="게시글 상세 정보 얻는 API")
    @ApiResponses(value= {
            @ApiResponse(code=200, message = "성공적으로 요청이 처리되었습니다.", response=BoardDTO.class),
            @ApiResponse(code=404, message = "잘못된 요청입니다"),
            @ApiResponse(code=500, message = "서버에서 오류가 발생했습니다")
